...
export default function DepartmentEditPage() {
  const [form, setForm] = useState<DepartmentForm>({ title: "" });
  ...
  useEffect(() => {
    if (!params?.id) return;
    apiGet<DepartmentForm>(`/api/departments/${params.id}`)
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await apiPatch(`/api/departments/${params.id}`, { title: form.title.trim() });
    ...
    <div className="flex min-h-screen bg-linear-to-b from-zinc-950 via-zinc-950 to-zinc-900 p-8">
      <div className="w-full max-w-4xl space-y-8 mx-auto">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-3xl font-bold text-zinc-50">Редактирование отдела</h1>
            <p className="text-zinc-400 mt-2 text-sm">Измените наименование отдела.</p>
          </div>
