...
export default function IndividualEditPage() {
  ...
  useEffect(() => {
    if (!params?.id) return;
    apiGet<any>(`/api/individuals/${params.id}`)
      .then((data) => {
        if (!data) {
          setForm({
            lastName: "",
      ...
      .catch((err) => console.error("Failed to load individual", err))
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await apiPatch(`/api/individuals/${params.id}`, {
    ...
    <div className="flex min-h-screen bg-linear-to-b from-zinc-950 via-zinc-950 to-zinc-900 p-8">
      <div className="w-full max-w-5xl space-y-8 mx-auto">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-3xl font-bold text-zinc-50">Редактирование сотрудника</h1>
            <p className="text-zinc-400 mt-2 text-sm">Измените ФИО, адрес регистрации и паспортные данные.</p>
          </div>
        ...
        <section className="space-y-4 rounded-xl border border-zinc-800 bg-zinc-900/80 p-6 shadow-xl shadow-black/20">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="space-y-1">
              ...
              <p className="text-xs text-zinc-400">Данные из справочника сотрудников.</p>
            </div>
