...
export default function AccountEditPage() {
  const [form, setForm] = useState<AccountForm>({ number: "", bank: "", bic: "" });
  ...
  useEffect(() => {
    if (!params?.id) return;
    apiGet<AccountForm>(`/api/accounts/${params.id}`)
      .then((data) => setForm(data ?? { number: "", bank: "", bic: "" }))
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await apiPatch(`/api/accounts/${params.id}`, {
    ...
    <div className="flex min-h-screen bg-linear-to-b from-zinc-950 via-zinc-950 to-zinc-900 p-8">
      <div className="w-full max-w-4xl space-y-8 mx-auto">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-3xl font-bold text-zinc-50">Редактирование счёта</h1>
            <p className="text-zinc-400 mt-2 text-sm">Измените номер, банк и БИК.</p>
          </div>
