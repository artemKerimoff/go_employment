...
import (
	...
	"github.com/example/micro/service-template/internal/data"
	"github.com/example/micro/service-template/internal/server"
...
func main() {
	...
	if err := svc.Seed(ctx); err != nil {
		logger.Warn("seed", slog.Any("err", err))
	}

	grpcSrv, err := server.NewGRPC(cfg, svc)
	if err != nil {
