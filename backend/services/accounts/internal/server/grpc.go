...
func New(cfg config.Config, svc accountspb.AccountsServiceServer) (*GRPCServer, error) {
    lis, err := net.Listen("tcp", fmt.Sprintf(":%d", cfg.GRPCPort))
    if err != nil {
        return nil, fmt.Errorf("listen: %w", err)
    }
    srv := grpc.NewServer()
    accountspb.RegisterAccountsServiceServer(srv, svc)
...
// HealthCheck helper for manual status updates.
func (s *GRPCServer) SetServing(serving bool) {
    hs := s.srv.GetServiceInfo()
    _ = hs // kept for potential use; placeholder to avoid unused warnings
}
